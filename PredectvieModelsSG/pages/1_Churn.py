# Libraries & Import data output modél 
import data
# ------------------------
import functools
...
# Config
st.set_page_config(page_title='Modéle CHURN', page_icon=':bar_chart:', layout='wide')

# Title

st.title("Modéle CHURN :bar_chart:")
...
def main() -> None:
    ...
    with st.expander("Description du Modéle"):
        st.write(Path("README-CHURN.md").read_text())

    st.subheader("Télécharger votre Output .CSV")
    ...
    # Sidbar - selection du cannal
    st.sidebar.subheader("Filter Vos Cannaux")
    accounts = list(df.account_name.unique())
    account_selections = st.sidebar.multiselect(
        "Selectionner votre Canal pour visualisation:", options=accounts, default=accounts
    )
    # Sidbar - selection de la DR
    st.sidebar.subheader("Filter votre Region DR")
    symbols = list(df.loc[df.account_name.isin(account_selections), "symbol"].unique())
    symbol_selections = st.sidebar.multiselect(
        "Selectionner votre DR pour visualisation", options=symbols, default=symbols
    )

    df = filter_data(df, account_selections, symbol_selections)
    st.subheader("Données Cible et Région sélectionnées")
